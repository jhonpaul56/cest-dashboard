...
function TrainingForm({ initial, onSave, onCancel }) {
  const blank = {
    date: "", year: "", municipality: "", community: "",
  ...
  const toggleBenef = k => set(
    "beneficiaryTypes",
    f.beneficiaryTypes.includes(k)
      ? f.beneficiaryTypes.filter(x => x !== k)
      : [...f.beneficiaryTypes, k]
  );
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
          {Object.entries(BENEF_TYPES).map(([k, v]) => (
            <button key={k} onClick={() => toggleBenef(k)} style={{
              background: f.beneficiaryTypes.includes(k) ? "#0369a1" : "#f3f4f6",
              border: `1px solid ${f.beneficiaryTypes.includes(k) ? "#0369a1" : "#d1d5db"}`,
              color: f.beneficiaryTypes.includes(k) ? "#fff" : "#374151",
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
          {[["male","Male"],["female","Female"],["total","Total"]].map(([k, l]) => (
            <div key={k}>
              ...
              <input
                type="number" value={f.beneficiaries[k]} min="0"
...
export default function TrainingsPage() {
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      {toast && (
        <div style={{
          position: "fixed", top: 18, right: 18, zIndex: 4000,
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16, flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          <input
            ...
            placeholder="Search training, community, municipality..."
            style={{ ...IS, minWidth: 240, boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}
          />
          ...
          <button
            ...
            style={{ background: "#1e40af", border: "none", color: "#fff", borderRadius: 8, padding: "9px 18px", fontSize: 13, fontWeight: 800, cursor: "pointer", whiteSpace: "nowrap" }}
          >＋ Add Training</button>
        </div>
      </div>
      ...
      <div style={{ background: "#fff", borderRadius: 12, boxShadow: "0 1px 8px rgba(0,0,0,0.06)", overflow: "hidden" }}>
        <div style={{ overflowX: "auto" }}>
          <table className="cest-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead>
              <tr>
                <th rowSpan={2} style={{ ...thBase, verticalAlign: "middle", textAlign: "center",  minWidth: 40  }}>No.</th>
                <th rowSpan={2} style={{ ...thBase, verticalAlign: "middle", textAlign: "center",  minWidth: 80  }}>Date / Year</th>
                <th rowSpan={2} style={{ ...thBase, verticalAlign: "middle", textAlign: "left",    minWidth: 120 }}>Municipality / City</th>
                <th rowSpan={2} style={{ ...thBase, verticalAlign: "middle", textAlign: "left",    minWidth: 120 }}>Community / Beneficiaries</th>
                <th rowSpan={2} style={{ ...thBase, verticalAlign: "middle", textAlign: "left",    minWidth: 180 }}>List of Trainings Conducted</th>
                <th rowSpan={2} style={{ ...thBase, verticalAlign: "middle", textAlign: "right",   minWidth: 100 }}>Budget</th>
                <th rowSpan={2} style={{ ...thBase, verticalAlign: "middle", textAlign: "center",  minWidth: 100 }}>MOV's</th>
                <th rowSpan={2} style={{ ...thBase, verticalAlign: "middle", textAlign: "center",  minWidth: 130, background: "#1e3a8f" }}>CEST 2.0 Components</th>
                <th colSpan={Object.keys(BENEF_TYPES).length} style={gTH("#0369a1")}>CEST 2.0 Beneficiaries</th>
                <th colSpan={3} style={gTH("#166534")}>No. of Beneficiaries</th>
                <th rowSpan={2} style={{ ...thBase, verticalAlign: "middle", textAlign: "center",  minWidth: 80  }}>Actions</th>
              </tr>
              <tr>
                {Object.values(BENEF_TYPES).map(v => (
                  <th key={v} style={sTH("#0369a1")}>{v}</th>
                ))}
                <th style={sTH("#166534")}>Male</th>
                <th style={sTH("#166534")}>Female</th>
                <th style={{ ...sTH("#166534"), fontWeight: 900 }}>Total</th>
              </tr>
            </thead>
            ...
            <tbody>
              {filtered.map((t, i) => (
                <tr key={t.id} style={{ background: i % 2 === 0 ? "#fff" : "#f8faff", borderBottom: "1px solid #f3f4f6" }}>
                  <TD className="col-center" style={{ color: "#9ca3af" }}>{t.id}</TD>
                  <TD className="col-center" style={{ fontWeight: 700, color: "#1e40af", whiteSpace: "nowrap" }}>
                    {t.date ? <div style={{ fontSize: 11 }}>{t.date}</div> : null}
                    {t.year ? <div>{t.year}</div> : null}
                  </TD>
                  <TD className="col-text" style={{ color: "#1e40af", fontWeight: 600 }}>{t.municipality}</TD>
                  <TD className="col-text">{t.community}</TD>

                  {/* Training title — pre-wrap para lumabas ang multiline */}
                  ...
                  <TD className="col-right" style={{ whiteSpace: "nowrap", color: "#16a34a", fontWeight: 700 }}>
                    {t.budget !== "" && t.budget != null ? `₱${Number(t.budget).toLocaleString()}` : ""}
                  </TD>
                  <TD className="col-center">{t.moa}</TD>
                  ...
                  {Object.keys(BENEF_TYPES).map(k => (
                    <TD key={k} className="col-center">
                      {(t.beneficiaryTypes || []).includes(k)
                        ? <span style={{ color: "#0369a1", fontWeight: 900, fontSize: 14 }}>✓</span>
                        : ""}
                    </TD>
                  ))}

                  <TD className="col-center">{t.beneficiaries?.male   || ""}</TD>
                  <TD className="col-center">{t.beneficiaries?.female || ""}</TD>
                  <TD className="col-center" style={{ fontWeight: 700 }}>{t.beneficiaries?.total || ""}</TD>

                  <TD className="col-center">
                    <div style={{ display: "flex", gap: 4, justifyContent: "center" }}>
                      <button
                        onClick={() => setModal({ type: "edit", data: t })}
                        style={{ background: "#eff6ff", border: "1px solid #bfdbfe", color: "#1e40af", borderRadius: 5, padding: "3px 8px", fontSize: 11, cursor: "pointer" }}
                      >Edit</button>
                      <button
                        onClick={() => setModal({ type: "delete", data: t })}
                        style={{ background: "#fef2f2", border: "1px solid #fecaca", color: "#ef4444", borderRadius: 5, padding: "3px 8px", fontSize: 11, cursor: "pointer" }}
                      >Del</button>
                    </div>
                  </TD>
                </tr>
              ...
              {!filtered.length && (
                <tr>
                  <td
                    colSpan={7 + 1 + Object.keys(BENEF_TYPES).length + 3 + 1}
            ...
            <tfoot>
              <tr style={{ background: "#1e3a5f", color: "#fff", fontWeight: 800 }}>
                <td colSpan={5} style={{ padding: "8px 12px", fontSize: 12, textAlign: "left" }}>
                  Total No. of Trainings Conducted: {filtered.length}
                </td>
                <td style={{ padding: "8px 12px", fontSize: 12, textAlign: "right", whiteSpace: "nowrap" }}>
                  ₱{totalBudget.toLocaleString()}
                </td>
                <td colSpan={1 + Object.keys(BENEF_TYPES).length + 4} style={{ padding: "8px 12px" }} />
              </tr>
            </tfoot>
          </table>
        </div>
